...
non.wrapped = gf_col(fct_reorder(state, trump_vote)~trump_vote/1000000, data = subset(election, trump_vote >2000000)) %>%
  gf_labs(y = "", x = "Trump Votes (Millions)", title = "States Original") %>%
  gf_theme(theme_minimal())

wrapped = gf_col(fct_reorder(state, trump_vote)~trump_vote/1000000, data = subset(election, trump_vote >2000000)) %>%
  gf_refine(scale_y_discrete(labels = label_wrap(5))) %>%
  gf_labs(y = "", x = "Trump Votes (Millions)", title = "States Wrapped") %>%
  gf_theme(theme_minimal())

grid.arrange(non.wrapped, wrapped, nrow = 1)
```
